...
# Heun's method
Heun = function(T, y_0, v_0, theta, N) {
  ...
  for (i in 1:N) {
    ...
    x[i+1] = x[i] + (dt/2) * (v_x[i] + v_xpred)
    y[i+1] = y[i] + (dt/2) * (v_y[i] + v_ypred)
    v_x[i+1] = v_x[i] + (dt/2) * (g_x + g_x)
    v_y[i+1] = v_y[i] + (dt/2) * (g_y + g_y)
    l = c(l, y[i+1])
    s = c(s, x[i+1])
  }
  ...
  cd_plot_projectile(
    filepath = filepath,
    title = "Projectile Trajectory: Heun's Method vs Analytical Solution",
    method_label = "Heun's Method",
    x_numeric = s,
    y_numeric = l,
